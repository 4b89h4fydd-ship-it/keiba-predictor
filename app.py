...
app = FastAPI(title="競馬展開AI", version="7.2-production-v58-horse-modal-swipe")
...
<link rel="manifest" href="/manifest-v58.webmanifest">
<link rel="stylesheet" href="/styles-v58.css">
<title>競馬展開AI</title>
</head>
<body>
<div id="app"><div class="boot">競馬展開AIを起動中…</div></div>
<script src="/app-v58.js"></script>
</body>
</html>"""
...
/* v58 — horse modal and swipe navigation */
...
function cacheKey(d){return "keiba:v58:races:"+d}
...
function header(title,back,sub){return '<header class="header"><div class="header-row">'+(back?'<button data-action="back">‹</button>':'')+'<div class="header-title"><h1>'+esc(title)+'</h1>'+(sub?'<small>'+esc(sub)+'</small>':'')+'</div><span class="build-badge">v58</span><button data-action="reload">↻</button></div></header>'}
...
function markStoreKey(r,no){return'keiba:v58:mark:'+[(r&&r.circuit)||'',(r&&r.date)||'',(r&&r.track)||'',(r&&r.raceNumber)||'',no||''].join('|')}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=58" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence · v58</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function runnerStyleSection(r,p){var rows=sortedHorseRows(p.rows);return'<section class="card runner-map-card"><div class="section-title-row"><div><div class="section-title">出走馬表 × 脚質マップ</div><div class="section-sub">馬番順。印はタップで切替。馬名を押すと馬ごとの基本情報を表示、横スワイプで前後の馬へ切替</div></div><span class="map-hint">逃 / 先 / 差 / 追 / 下</span></div>'+stylePositionMap(r,p)+'<div class="runner-style-list">'+rows.map(function(x){var h=x.horse,ps=styleDisplayPcts(x),mx=Math.max.apply(null,ps),fade=x.styleSamples?Math.round(x.fade*100):null,manualMark=rowMark(r,x),markLabel=manualMark||'--',bodyTxt=horseBodyWeightText(h),styleTxt=x.expected||x.pastStyle||'不明';return'<details class="runner-style"><summary><div class="runner-summary-table"><div class="runner-summary-no">'+badge(h)+'</div><button type="button" class="runner-mark-toggle '+(manualMark?'':'empty')+'" data-mark-toggle="1" data-horse-no="'+esc(h.horseNumber)+'"><b>'+esc(markLabel)+'</b><small>印</small></button><div class="runner-summary-main"><div class="runner-summary-topline"><button type="button" class="runner-name-btn" data-horse-open="'+esc(h.horseNumber)+'"><span class="horse-name">'+esc(h.name)+'</span>'+(bodyTxt?'<span class="runner-weight-inline">('+esc(bodyTxt)+')</span>':'')+'</button></div><div class="runner-summary-meta compact"><span>'+esc(h.sex||'—')+esc(h.age||'—')+'</span><span>'+esc(styleTxt)+'</span><span>'+esc(h.jockey||'騎手不明')+'</span><span>'+esc(h.carriedWeight||'—')+'kg</span></div></div><div class="runner-grade-side"><small>総合評価</small><b class="'+gradeClass(x.overallGrade)+'">'+esc(x.overallGrade||'保留')+'</b><span>（'+(x.overallScore==null?'—':esc(x.overallScore))+'）</span></div><span class="open-caret">⌄</span></div><div class="runner-position-title"><b>位置取り指標</b><span>逃 / 先 / 差 / 追 / 下</span></div><div class="style-rate-grid five-rates">'+styleCell('逃',ps[0],ps[0]===mx,'front')+styleCell('先',ps[1],ps[1]===mx,'stalk')+styleCell('差',ps[2],ps[2]===mx,'mid')+styleCell('追',ps[3],ps[3]===mx,'close')+styleCell('下',fade,fade!=null&&fade>=55,'fade')+'</div><div class="detail-tap-note">詳細は馬名タップ、または行タップで開く</div></summary>'+runnerDetailBody(r,p,x)+'</details>'}).join('')+'</div></section>'}
...
@app.get("/build")
def build_info():
    return {"build":"v58","appVersion":"7.2-production-v58-horse-modal-swipe","recentRuns":5,"marks":True,"horseModalSwipe":True,"grade":"S/A/B/C + ◎○▲☆△注","history":"NAR + JRA公式/central feed auto-search","pastRaceOpen":True,"historySpeed":"instant-render+background-fill+parallel-prefetch","narWorkers":int(os.getenv("NAR_HISTORY_WORKERS","6")),"centralWorkers":int(os.getenv("CENTRAL_HISTORY_WORKERS","6"))}
...
@app.get("/manifest-v58.webmanifest")
def manifest():
