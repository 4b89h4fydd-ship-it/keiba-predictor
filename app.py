...
app = FastAPI(title="競馬展開AI", version="4.0-production-v20-safe")
...
function renderHome(){var all=state.circuit==="中央"?CENTRAL:LOCAL,venues=[],i,t,c;for(i=0;i<all.length;i++){t=all[i];c=state.races.filter(function(r){return r.circuit===state.circuit&&r.track===t}).length;if(c)venues.push([t,c])}var live=liveRaces(),liveHtml="";if(state.loading)liveHtml='<div class="empty">読込中…</div>';else if(state.date!==today())liveHtml='<div class="empty">当日を選ぶとリアルタイム表示します</div>';else if(live.length)liveHtml='<div class="live-grid">'+live.map(function(r){return'<button class="live-race" data-race="'+esc(r.id)+'"><div class="live-top"><span class="live-track">'+esc(r.track)+' '+esc(r.raceNumber)+'R</span>'+liveTag(r)+'</div><div class="live-title">'+esc(r.title||"")+'</div><div class="live-time">'+timeHtml(r)+'</div></button>'}).join("")+'</div>';else liveHtml='<div class="empty">直近の未確定レースはありません</div>';var nx=nextRace(),ai=nx?'<div class="ai-kicker">AI PACE PREDICTION</div><div class="ai-title">AI展開予想</div><div class="ai-next">次のレース：'+esc(nx.track)+' '+esc(nx.raceNumber)+'R　'+timeHtml(nx)+'</div><div class="ai-actions"><button class="ai-primary" data-action="pace-next">展開を見る</button><button class="ai-secondary" data-action="pace-pick">選択</button></div>':'<div class="ai-kicker">AI PACE PREDICTION</div><div class="ai-title">AI展開予想</div><div class="ai-next">未確定レースの取得待ち</div>';var vh=venues.length?'<div class="venue-grid">'+venues.map(function(v){return'<button class="venue" data-track="'+esc(v[0])+'"><div class="name">'+esc(v[0])+'</div><div class="count">'+v[1]+'レース</div></button>'}).join("")+'</div>':'<div class="empty">'+(state.error?esc(state.error):(state.circuit==="中央"?"中央データ源未接続、または開催データなし":"この日の取得データはありません"))+'</div>';return'<div class="shell">'+header("レース選択",false)+ '<main class="main"><section class="card"><div class="home-heading">日付・区分</div><div class="setup-grid"><div><div class="label">日付</div><input id="date" class="date" type="date" value="'+esc(state.date)+'"></div><div><div class="label">区分</div><div class="segment"><button data-circuit="中央" class="'+(state.circuit==="中央"?"active":"")+'">中央</button><button data-circuit="地方" class="'+(state.circuit==="地方"?"active":"")+'">地方</button></div></div></div></section><section class="card"><div class="row between"><h2>リアルタイムのレース</h2><span class="live-dot">LIVE</span></div>'+liveHtml+'</section><section class="card ai-card">'+ai+'</section><section class="card"><div class="row between"><h2>開催場</h2><span class="pill">'+state.races.filter(function(r){return r.circuit===state.circuit}).length+'R</span></div>'+vh+'</section></main></div>'}
...
def health():
    ...
    return {
        "status":"ok", "mode":"production-v20-safe", "historyStarted":_history_started,
