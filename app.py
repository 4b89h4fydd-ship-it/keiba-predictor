...
app = FastAPI(title="競馬展開AI", version="5.1-production-v35-strong-stylemap")
...
function cacheKey(d){return "keiba:v34:races:"+d}
...
function styleRates(h,r){var rs=(h.recentRaces||[]).slice(0,12),rw=recencyWeights(rs.length),c=[0,0,0,0],den=0,i,rr,p,pos,fs,norm,ww,delta,rel;for(i=0;i<rs.length;i++){rr=rs[i];p=rr.cornerPositions||[];pos=n(p[0],0);if(!pos)continue;fs=raceField(rr);norm=(pos-1)/Math.max(1,fs-1);delta=Math.abs(n(rr.distance)-n(r&&r.distance));rel=1;if(delta<=100)rel*=1.16;else if(delta<=300)rel*=1.05;else if(delta>=700)rel*=.80;if(r&&rr.track===r.track)rel*=1.10;if(r&&sameCondition(rr.condition,r.condition))rel*=1.04;ww=rw[i]*rel;den+=ww;if(pos===1)c[0]+=ww;else if(pos<=3||norm<=.23)c[1]+=ww;else if(norm<=.64)c[2]+=ww;else c[3]+=ww}if(!den)return{front:.05,stalk:.25,mid:.45,close:.25};return{front:c[0]/den,stalk:c[1]/den,mid:c[2]/den,close:c[3]/den}}
...
function scenarioPlan(r,rows,sc,suit){var scenario=sc.slice().sort(function(a,b){return b.prob-a.prob})[0],code=scenario.code,p=pressureInfo(rows),ct=courseTraits(r),S=[{},{},{},{},{},{}],L=[{},{},{},{},{},{}],i,x,no,q,scfit,markScore={};for(i=0;i<rows.length;i++){x=rows[i];no=n(x.horse.horseNumber);q=p[no]||{};S[0][no]=x.breakSkill*.40+x.goProb*.38+x.needLead*.08+x.jockeyFront*.06+(q.freeOuter||0)-q.conflict*.04;L[0][no]=clamp(Math.round((x.draw-.5)*5),-3,3);S[1][no]=S[0][no]*.44+x.goProb*.18+x.hold*.12+x.turnSkill*.10+x.flexibility*.07+x.ability*.09-q.conflict*.06-x.outer*ct.turnLoad*.04;L[1][no]=clamp(L[0][no]+(x.edge&&x.goProb>.55?-1:0)+(q.sandwich&&x.flexibility<.45?1:0),-3,3);scfit=scenarioSuit(x,code,p);S[2][no]=S[1][no]*.31+x.ability*.18+x.hold*.15+x.stamina*.12+x.stalk*.08+x.mid*.04+scfit*.12+(code==='A'?x.goProb*.07:0)+(code==='C'?x.move*.06:0);L[2][no]=clamp(L[1][no]+(x.move>.50&&x.goProb<.45?1:0),-3,3);S[3][no]=S[2][no]*.29+x.ability*.18+x.move*.17+x.turnSkill*.12+scfit*.15+x.latePower*.06+x.hold*.03;L[3][no]=clamp(L[2][no]+(x.move>.55?1:0),-3,3);S[4][no]=S[3][no]*.27+scfit*.21+x.ability*.20+x.hold*.10+x.move*.10+x.latePower*.08+(1-x.fade)*.04;L[4][no]=clamp(L[3][no]+(x.latePower>.60?1:0),-3,3);var su=suit[no]||{rankScore:.5};markScore[no]=S[4][no]*.34+su.rankScore*.50+x.latePower*.08+x.stamina*.05+x.flexibility*.03;if(code==='A')markScore[no]+=x.goProb*.035+x.hold*.025;if(code==='C')markScore[no]+=x.move*.035+x.close*.025;S[5][no]=markScore[no];L[5][no]=L[4][no]}
var labels=['スタート','1角','向正面','3角','4角','直線'],packs=[],stages=[];for(i=0;i<6;i++){packs[i]=packScenarioStage(r,rows,S[i],L[i],i);stages.push({key:['start','first','back','turn3','turn4','straight'][i],label:labels[i],pack:packs[i]})}return{scenario:scenario,stages:stages,start:rowsFromPack(rows,packs[0]),corner:rowsFromPack(rows,packs[4]),straight:rowsFromPack(rows,packs[5]),markScore:markScore}}
function planMarks(rows,plan){var syms=['◎','○','▲','☆','△','注'],by={};for(var i=0;i<rows.length;i++)by[n(rows[i].horse.horseNumber)]=rows[i].horse;return(plan.stages[5].pack||[]).slice(0,syms.length).map(function(z,i){return[syms[i],by[n(z.no)]]})}
function predict(r){var rows=buildRows(r),occ=earlyOcc(r),sc=scenarioModel(r,rows),suit=suitability(rows,sc),plan=scenarioPlan(r,rows,sc,suit),marks=planMarks(rows,plan),pressure=pressureInfo(rows),i;for(i=0;i<sc.length;i++){var code=sc[i].code,candidates=rows.slice().sort(function(a,b){return scenarioSuit(b,code,pressure)-scenarioSuit(a,code,pressure)});sc[i].horses=candidates.slice(0,3).map(function(x){return x.horse})}var cov=mean(rows.map(function(x){return x.coverage}));return{rows:rows,occ:occ,scenarios:sc,marks:marks,plan:plan,suit:suit,coverage:cov,pressure:pressure}}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=35" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=35" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function roleDetail(label,stats,profile){stats=stats||{};profile=profile||{};var st=n(stats.starts)||n(profile.starts),w=n(stats.wins),s=n(stats.seconds),t=n(stats.thirds),pr=st?Math.round((w+s+t)/st*100):null;var bits=[];if(st)bits.push('出走 '+st);if(pr!=null)bits.push('複勝率 '+pr+'%');if(n(profile.track,0))bits.push('競馬場 '+Math.round(n(profile.track)*100)+'%');if(n(profile.distance,0))bits.push('距離 '+Math.round(n(profile.distance)*100)+'%');if(n(profile.condition,0))bits.push('馬場 '+Math.round(n(profile.condition)*100)+'%');if(n(profile.early3Rate,0))bits.push('序盤3番手内 '+Math.round(n(profile.early3Rate)*100)+'%');if(n(profile.leaderRate,0))bits.push('逃げ '+Math.round(n(profile.leaderRate)*100)+'%');return'<div class="role-box"><b>'+esc(label)+'</b><div>'+(bits.length?bits.join(' / '):'条件別成績データなし')+'</div></div>'}
function styleDisplayPcts(x){var raw=[x.front,x.stalk,x.mid,x.close],vals=[],sum=0,i,maxi=0;for(i=0;i<4;i++){vals[i]=Math.max(0,Math.round(raw[i]*100));sum+=vals[i];if(raw[i]>raw[maxi])maxi=i}vals[maxi]+=100-sum;return vals}
function styleCell(label,val,active,cls){return'<div class="style-rate '+cls+(active?' dominant':'')+'"><div class="style-rate-head"><span>'+label+'</span><b>'+val+'%</b></div><div class="style-bar"><i style="width:'+Math.max(2,val)+'%"></i></div></div>'}
function runnerStyleSection(r,p){var rows=p.rows.slice().sort(function(a,b){return n(a.horse.horseNumber)-n(b.horse.horseNumber)});return'<section class="card runner-map-card"><div class="section-title-row"><div><div class="section-title">出走馬表 × 脚質マップ</div><div class="section-sub">馬番順・脚質点なし。馬名を押すと全データを表示</div></div><span class="map-hint">逃 / 先 / 差 / 追</span></div><div class="runner-style-list">'+rows.map(function(x){var h=x.horse,fit=x.fit||{},cnt=fit.counts||{},j=h.jockeyProfile||{},t=h.trainerProfile||{},ps=styleDisplayPcts(x),mx=Math.max.apply(null,ps),ms=markSymbol(p,h.horseNumber);function fitVal(k){return Math.round(confidenceBlend(fit[k],cnt[k])*100)}var recent=(h.recentRaces||[]).slice(0,12);return'<details class="runner-style"><summary><div class="runner-line"><div class="runner-id">'+badge(h)+'</div><div class="runner-copy"><div class="runner-name-line">'+(ms?'<span class="runner-mark">'+ms+'</span>':'')+'<span class="horse-name">'+esc(h.name)+'</span><span class="expected">'+esc(x.expected)+'</span></div><div class="runner-subline"><span>'+esc(h.sex||'—')+esc(h.age||'—')+'</span><span>'+esc(h.carriedWeight||'—')+'kg</span><span>'+esc(h.jockey||'騎手不明')+'</span></div></div><span class="open-caret">⌄</span></div><div class="style-rate-grid">'+styleCell('逃',ps[0],ps[0]===mx,'front')+styleCell('先',ps[1],ps[1]===mx,'stalk')+styleCell('差',ps[2],ps[2]===mx,'mid')+styleCell('追',ps[3],ps[3]===mx,'close')+'</div></summary><div class="horse-detail"><div class="detail-heading">基本情報</div><div class="horse-info-grid"><div class="horse-info-cell"><small>馬番 / 枠</small><b>'+esc(h.horseNumber)+'番 / '+esc(h.frameNumber||frame(h))+'枠</b></div><div class="horse-info-cell"><small>性齢 / 斤量</small><b>'+esc(h.sex||'—')+esc(h.age||'—')+' / '+esc(h.carriedWeight||'—')+'kg</b></div><div class="horse-info-cell"><small>騎手</small><b>'+esc(h.jockey||'—')+'</b></div><div class="horse-info-cell"><small>調教師</small><b>'+esc(h.trainer||'—')+'</b></div><div class="horse-info-cell"><small>当時獲得賞金</small><b>'+fmtMoney(h.prizeMoneyAtRace)+'円</b></div><div class="horse-info-cell"><small>今回の位置想定</small><b>'+esc(x.expected)+'</b></div></div><div class="detail-heading">今回条件への適性</div><div class="fit-grid"><div class="fit-chip"><small>距離</small><b>'+fitVal('distance')+'</b></div><div class="fit-chip"><small>競馬場</small><b>'+fitVal('track')+'</b></div><div class="fit-chip"><small>馬場</small><b>'+fitVal('condition')+'</b></div><div class="fit-chip"><small>天候</small><b>'+fitVal('weather')+'</b></div><div class="fit-chip"><small>季節</small><b>'+fitVal('season')+'</b></div><div class="fit-chip"><small>相手レベル</small><b>'+fitVal('level')+'</b></div></div>'+roleDetail('騎手成績',h.jockeyStats,j)+roleDetail('調教師成績',h.trainerStats,t)+'<div class="recent-list-title">近走データ（最大12走）</div>'+(recent.length?recent.map(function(rr){return'<div class="recent"><div class="recent-head"><b>'+esc(rr.date)+' '+esc(rr.track)+' '+esc(rr.distance)+'m</b><strong>'+esc(rr.finish||'—')+'着</strong></div><div>'+fmtTime(rr.timeSeconds)+'　'+esc(rr.condition||'不明')+' / '+esc(rr.weather||'不明')+'</div><div class="muted">通過 '+esc((rr.cornerPositions||[]).join('-')||'—')+'　頭数 '+esc(rr.fieldSize||'—')+(n(rr.carriedWeight)>0?'　斤量 '+esc(rr.carriedWeight)+'kg':'')+(n(rr.racePrize1)>0?'　1着賞金 '+fmtMoney(rr.racePrize1):'')+'</div></div>'}).join(''):'<div class="empty compact">近走データなし</div>')+'</div></details>'}).join('')+'</div></section>'}
function scenarioProbabilitySection(p){var top=p.scenarios.slice().sort(function(a,b){return b.prob-a.prob})[0];return'<section class="card pace-card"><div class="section-title">展開予想</div><div class="scenario-prob-grid">'+p.scenarios.map(function(s){return'<div class="scenario-prob-card '+(s.code===top.code?'active':'')+'"><small>'+s.code+'</small><b>'+esc(s.title)+'</b><strong>'+Math.round(s.prob*100)+'%</strong></div>'}).join('')+'</div><div class="scenario-main-note"><span>本線シナリオ</span><b>'+esc(top.code)+' '+esc(top.title)+'　'+Math.round(top.prob*100)+'%</b></div></section>'}
function paceBoard(r,p){var hs=(r.horses||[]).slice().sort(function(a,b){return n(a.horseNumber)-n(b.horseNumber)}),cp=courseProfile(r),d=coursePathD(cp),straight=cp.shape==='straight',stages=p.plan.stages||[];return'<section class="card pace-card"><div class="pace-head"><div class="section-title" style="margin:0">本線シナリオの隊列</div><span class="pace-stage">'+esc(p.plan.scenario.code)+' '+esc(p.plan.scenario.title)+'</span></div><div id="pace-board" class="course-wrap"><div class="course-meta">'+esc(r.track)+'　'+esc(r.distance)+'m　'+esc(cp.turn)+(straight?'':'回り')+'</div><svg class="course-svg" viewBox="0 0 200 180" preserveAspectRatio="none" aria-hidden="true">'+(straight?'<path d="M18 92 L182 92" class="course-track-under"></path><path id="course-path" d="M18 92 L182 92" class="course-track"></path><path d="M18 92 L182 92" class="course-rail"></path>':'<path d="'+d+'" class="course-track-under"></path><path id="course-path" d="'+d+'" class="course-track"></path><path d="'+d+'" class="course-rail"></path>')+'<circle id="course-start-dot" class="course-start" cx="20" cy="90" r="3"></circle><line id="course-finish-line" class="course-finish" x1="174" y1="77" x2="174" y2="103"></line><text id="course-start-label" class="course-start-label" x="23" y="83">START</text><text class="course-finish-label" x="162" y="72">GOAL</text></svg>'+hs.map(function(h){var ms=markSymbol(p,h.horseNumber);return'<div class="course-runner pace-runner" data-horse="'+esc(h.horseNumber)+'" style="left:10%;top:50%">'+(ms?'<span class="course-mark">'+ms+'</span>':'')+badge(h)+'<span class="pace-name">'+esc(h.name)+'</span></div>'}).join('')+'<div id="course-order" class="pace-order-full">隊列を準備中</div></div><div class="pace-stage-tabs">'+stages.map(function(s,i){return'<button data-pace-stage="'+i+'" class="'+(i===0?'active':'')+'">'+esc(s.label)+'</button>'}).join('')+'</div><div class="muted" style="margin-top:7px">脚質マップ・枠順・テン・隣接圧力・距離変化・本線シナリオ・予想印を同じ計算から隊列化。各地点を押すとその位置関係を表示します。</div></section>'}
...
function drawPaceStage(idx){if(!state.race||!state.pred||!state.pred.plan||!state.pred.plan.stages)return;var stages=state.pred.plan.stages,st=stages[clamp(idx,0,stages.length-1)],r=state.race,cp=courseProfile(r),board=document.getElementById('pace-board'),path=document.getElementById('course-path');if(!st||!board||!path)return;var base=paceBaseFrac(r,idx),order=[],i,z,chip,frac;for(i=0;i<st.pack.length;i++){z=st.pack[i];chip=board.querySelector('[data-horse="'+z.no+'"]');if(cp.shape==='straight')frac=clamp(base-z.gap*.78,.025,.975);else frac=normFrac(base-cp.dir*z.gap);placeSimRunner(chip,path,board,frac,z.lane);order.push(z.no)}var ob=document.getElementById('course-order');if(ob)ob.innerHTML='<b>'+esc(st.label)+'</b><span>'+order.map(function(no){var h=horseByNo(r,no);return esc(no)+(h?' '+esc(h.name):'')}).join(' → ')+'</span>';var bs=document.querySelectorAll('[data-pace-stage]');for(i=0;i<bs.length;i++)bs[i].className=n(bs[i].getAttribute('data-pace-stage'))===idx?'active':''}
function initPaceBoard(){if(!state.pred||!state.pred.plan||!state.pred.plan.stages)return;var board=document.getElementById('pace-board'),path=document.getElementById('course-path');if(board&&path){var sf=courseStageFrac(state.race,0),total=path.getTotalLength(),sp=path.getPointAtLength(sf*total),sd=document.getElementById('course-start-dot'),sl=document.getElementById('course-start-label');if(sd){sd.setAttribute('cx',sp.x);sd.setAttribute('cy',sp.y)}if(sl){sl.setAttribute('x',Math.min(176,sp.x+4));sl.setAttribute('y',Math.max(10,sp.y-5))}}drawPaceStage(0)}
function load(){var d=state.date,seq=++state.requestSeq,cached=loadRaceCache(d);state.error=null;if(cached&&cached.length){state.races=cached;state.loading=false;render()}else{state.loading=true;render()}var attempts=0;function request(){attempts+=1;fetch('/api/v1/races?date='+encodeURIComponent(d)+'&v=34',{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('API '+res.status);return res.json()}).then(function(body){if(seq!==state.requestSeq||state.date!==d)return;var rows=Array.isArray(body)?body:(body.races||[]);state.races=rows;saveRaceCache(d,rows);state.loading=false;state.error=null;render()}).catch(function(){if(seq!==state.requestSeq||state.date!==d)return;if(attempts<3){setTimeout(request,700*attempts);return}state.loading=false;state.error=null;render()})}request()}
                ...
                horses.append(
                    {
                        ...
                        "recentRaces": self.recent_races(e["name"], iso_date, 12),
                    }
                )
            finishers = []
            for e in entries:
