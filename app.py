...
app = FastAPI(title="競馬展開AI", version="5.2-production-v36-deep-pace")
...
function cacheKey(d){return "keiba:v36:races:"+d}
...
function styleRates(h,r){var rs=(h.recentRaces||[]).slice(0,12),rw=recencyWeights(rs.length),c=[0,0,0,0],den=0,earlyDen=0,early3=0,moved3=0,tempoVals=[],tempoW=[],i,rr,p,pos,fs,norm,ww,delta,rel,j,bestLater;for(i=0;i<rs.length;i++){rr=rs[i];p=rr.cornerPositions||[];pos=n(p[0],0);if(!pos)continue;fs=raceField(rr);norm=(pos-1)/Math.max(1,fs-1);delta=Math.abs(n(rr.distance)-n(r&&r.distance));rel=1;if(delta<=100)rel*=1.22;else if(delta<=300)rel*=1.08;else if(delta>=700)rel*=.74;if(r&&rr.track===r.track)rel*=1.10;if(r&&sameCondition(rr.condition,r.condition))rel*=1.05;ww=rw[i]*rel;den+=ww;earlyDen+=ww;if(pos===1)c[0]+=ww;else if(pos<=3||norm<=.22)c[1]+=ww;else if(norm<=.62)c[2]+=ww;else c[3]+=ww;if(pos<=3)early3+=ww;bestLater=99;for(j=1;j<p.length;j++){var z=n(p[j],0);if(z&&z<bestLater)bestLater=z}if(pos>3&&bestLater<=3)moved3+=ww;tempoVals.push(clamp(1-(pos-1)/Math.max(3,fs-1),0,1));tempoW.push(ww)}if(!den)return{front:.05,stalk:.25,mid:.45,close:.25,early3:.25,moved3:.08,ten:.5,samples:0};return{front:c[0]/den,stalk:c[1]/den,mid:c[2]/den,close:c[3]/den,early3:earlyDen?early3/earlyDen:0,moved3:earlyDen?moved3/earlyDen:0,ten:weightedRate(tempoVals,tempoW,.5),samples:tempoVals.length}}
function tenScore(h,r){var rt=styleRates(h,r),rs=(h.recentRaces||[]).slice(0,6),w=recencyWeights(rs.length),v=[],ws=[],i,rr,p,fs,delta,rel;for(i=0;i<rs.length;i++){rr=rs[i];p=rr.cornerPositions||[];if(!n(p[0]))continue;fs=raceField(rr);delta=Math.abs(n(rr.distance)-n(r&&r.distance));rel=delta<=100?1.18:(delta<=300?1.05:.88);v.push(clamp(1-(n(p[0])-1)/Math.max(3,fs-1),0,1));ws.push((w[i]||1)*rel)}return clamp(weightedRate(v,ws,rt.ten)*.72+rt.front*.18+rt.early3*.10,0,1)}
...
function buildRows(r){var hs=r.horses||[],tmp=[],speeds=[],prizes=[],i,h,rt,pt,sr,fit,ct=courseTraits(r),cp=courseProfile(r),field=Math.max(1,hs.length);for(i=0;i<hs.length;i++){h=hs[i];rt=styleRates(h,r);pt=stylePoint(rt);sr=speedRaw(h,n(r.distance));fit=conditionFit(h,r);tmp.push({horse:h,front:rt.front,stalk:rt.stalk,mid:rt.mid,close:rt.close,early3:rt.early3,moved3:rt.moved3,styleSamples:rt.samples,ten:tenScore(h,r),fade:fadeRate(h),move:moveRate(h),hold:holdRate(h),yieldFlex:yieldFlex(h),breakRel:breakReliability(h),lateGain:lateGainScore(h),posCons:positionConsistency(h),collapse:earlyCollapseSeverity(h),score:pt,pastStyle:styleName(pt),expected:styleName(pt),speedRaw:sr,fit:fit});speeds.push(sr);prizes.push(n(h.prizeMoneyAtRace))}
...
function planMarks(rows,plan,suit,sc){var syms=['◎','○','▲','☆','△','注'],pressure=pressureInfo(rows),ranked=rows.slice().map(function(x){var su=suit[n(x.horse.horseNumber)]||{rankScore:.5,scenario:[]},vals=(su.scenario||[]).map(function(z){return z.score}),robust=vals.length?Math.min.apply(null,vals):.5,top=plan.markScore[n(x.horse.horseNumber)]||0,q=pressure[n(x.horse.horseNumber)]||{},score=su.rankScore*.50+top*.18+x.ability*.12+robust*.08+x.coverage*.04+x.holdFront*.04+x.latePower*.04-(q.conflict||0)*.012;if(x.goProb>.55)score-=x.fade*.025;return{x:x,score:score}}).sort(function(a,b){return b.score-a.score||n(a.x.horse.horseNumber)-n(b.x.horse.horseNumber)});return ranked.slice(0,syms.length).map(function(z,i){return[syms[i],z.x.horse]})}
function predict(r){var rows=buildRows(r),occ=earlyOcc(r),sc=scenarioModel(r,rows),suit=suitability(rows,sc),plan=scenarioPlan(r,rows,sc,suit),marks=planMarks(rows,plan,suit,sc),pressure=pressureInfo(rows),i;for(i=0;i<sc.length;i++){var code=sc[i].code,candidates=rows.slice().sort(function(a,b){return scenarioSuit(b,code,pressure)-scenarioSuit(a,code,pressure)});sc[i].horses=candidates.slice(0,3).map(function(x){return x.horse})}var cov=mean(rows.map(function(x){return x.coverage}));return{rows:rows,occ:occ,scenarios:sc,marks:marks,plan:plan,suit:suit,coverage:cov,pressure:pressure}}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=36" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=36" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function roleDetail(label,stats,profile){stats=stats||{};profile=profile||{};var st=n(stats.starts)||n(profile.starts),w=n(stats.wins),s=n(stats.seconds),t=n(stats.thirds),wr=st?Math.round(w/st*100):null,rr=st?Math.round((w+s)/st*100):null,pr=st?Math.round((w+s+t)/st*100):null,bits=[];if(st)bits.push('出走 '+st);bits.push('1着 '+w);bits.push('2着 '+s);bits.push('3着 '+t);if(wr!=null)bits.push('勝率 '+wr+'%');if(rr!=null)bits.push('連対 '+rr+'%');if(pr!=null)bits.push('複勝 '+pr+'%');if(n(profile.track,0))bits.push('競馬場 '+Math.round(n(profile.track)*100)+'%');if(n(profile.distance,0))bits.push('距離 '+Math.round(n(profile.distance)*100)+'%');if(n(profile.condition,0))bits.push('馬場 '+Math.round(n(profile.condition)*100)+'%');if(n(profile.early3Rate,0))bits.push('序盤3番手内 '+Math.round(n(profile.early3Rate)*100)+'%');if(n(profile.leaderRate,0))bits.push('逃げ '+Math.round(n(profile.leaderRate)*100)+'%');return'<div class="role-box"><b>'+esc(label)+'</b><div>'+(bits.length?bits.join(' / '):'条件別成績データなし')+'</div></div>'}
...
function positionBucket(x){if(x.expected==="逃げ候補")return"逃げ候補";if(x.expected==="先行")return"先行";if(x.expected==="好位")return"好位";if(x.expected==="後方")return"後方";return"中団"}
function stylePositionMap(r,p){var rows=p.rows||[],labels=['逃げ候補','先行','好位','中団','後方'],field=Math.max(1,(r.horses||[]).length),html='<div class="style-position-map"><div class="style-map-axis"><span>内枠</span><b>今回の枠順 × 想定位置</b><span>外枠</span></div>';for(var j=0;j<labels.length;j++){var lab=labels[j];html+='<div class="style-lane"><div class="style-lane-label">'+lab+'</div><div class="style-lane-track">';for(var i=0;i<rows.length;i++){var x=rows[i],h=x.horse;if(positionBucket(x)!==lab)continue;var left=field<=1?50:6+(n(h.horseNumber)-1)/Math.max(1,field-1)*88,shift=x.pastStyle!==x.expected&&!(x.pastStyle==='先行'&&x.expected==='好位'),ms=markSymbol(p,h.horseNumber);html+='<span class="style-map-horse'+(shift?' shifted':'')+'" style="left:'+left+'%" title="'+esc(h.name)+'｜過去 '+esc(x.pastStyle)+' → 今回 '+esc(x.expected)+'">'+badge(h)+(ms?'<span class="map-mark">'+ms+'</span>':'')+'</span>'}html+='</div></div>'}html+='<div class="style-map-note"><b>水色縁</b>＝過去脚質から今回条件で位置想定が動いた馬。馬番位置は内→外の枠順を維持。</div></div>';return html}
function runnerStyleSection(r,p){var rows=p.rows.slice().sort(function(a,b){return a.score-b.score||n(a.horse.horseNumber)-n(b.horse.horseNumber)});return'<section class="card runner-map-card"><div class="section-title-row"><div><div class="section-title">出走馬表 × 脚質マップ</div><div class="section-sub">脚質点昇順。過去脚質と今回の枠順・テン・隣接圧力を分離して表示</div></div><span class="map-hint">逃 / 先 / 差 / 追 / 下</span></div>'+stylePositionMap(r,p)+'<div class="runner-style-list">'+rows.map(function(x){var h=x.horse,fit=x.fit||{},cnt=fit.counts||{},j=h.jockeyProfile||{},t=h.trainerProfile||{},ps=styleDisplayPcts(x),mx=Math.max.apply(null,ps),fade=Math.round(x.fade*100),ms=markSymbol(p,h.horseNumber),q=p.pressure&&p.pressure[n(h.horseNumber)]||{};function fitVal(k){return Math.round(confidenceBlend(fit[k],cnt[k])*100)}function fitText(k){return fitVal(k)+' / '+n(cnt[k])+'走'}var recent=(h.recentRaces||[]).slice(0,12),distTxt=x.shorten?'短縮 '+Math.abs(x.distanceChange)+'m':(x.lengthen?'延長 '+Math.abs(x.distanceChange)+'m':'同距離帯'),pressureTxt=(q.sandwich?'逃げハサミ警戒':((q.leftHot||q.rightHot)?'逃げ横あり':'隣接圧力弱め'));return'<details class="runner-style"><summary><div class="runner-line"><div class="runner-id">'+badge(h)+'</div><div class="runner-copy"><div class="runner-name-line">'+(ms?'<span class="runner-mark">'+ms+'</span>':'')+'<span class="horse-name">'+esc(h.name)+'</span><span class="expected">'+esc(x.expected)+'</span></div><div class="runner-subline"><span>'+esc(h.sex||'—')+esc(h.age||'—')+'</span><span>'+esc(h.carriedWeight||'—')+'kg</span><span>'+esc(h.jockey||'騎手不明')+'</span></div></div><span class="open-caret">⌄</span></div><div class="style-rate-grid five-rates">'+styleCell('逃',ps[0],ps[0]===mx,'front')+styleCell('先',ps[1],ps[1]===mx,'stalk')+styleCell('差',ps[2],ps[2]===mx,'mid')+styleCell('追',ps[3],ps[3]===mx,'close')+styleCell('下',fade,fade>=55,'fade')+'</div><div class="runner-score-row"><div class="runner-score-chip"><small>脚質点</small><b>'+x.score.toFixed(2)+'</b></div><div class="runner-score-chip"><small>前へ行く</small><b>'+Math.round(x.goProb*100)+'%</b></div><div class="runner-score-chip"><small>テン</small><b>'+Math.round(x.ten*100)+'</b></div><div class="runner-score-chip"><small>前残り力</small><b>'+Math.round(x.frontStay*100)+'</b></div></div></summary><div class="horse-detail"><div class="detail-heading">基本情報</div><div class="horse-info-grid"><div class="horse-info-cell"><small>馬番 / 枠</small><b>'+esc(h.horseNumber)+'番 / '+esc(h.frameNumber||frame(h))+'枠</b></div><div class="horse-info-cell"><small>性齢 / 斤量</small><b>'+esc(h.sex||'—')+esc(h.age||'—')+' / '+esc(h.carriedWeight||'—')+'kg</b></div><div class="horse-info-cell"><small>騎手</small><b>'+esc(h.jockey||'—')+'</b></div><div class="horse-info-cell"><small>調教師</small><b>'+esc(h.trainer||'—')+'</b></div><div class="horse-info-cell"><small>当時獲得賞金</small><b>'+fmtMoney(h.prizeMoneyAtRace)+'円</b></div><div class="horse-info-cell"><small>今回の位置想定</small><b>'+esc(x.pastStyle)+' → '+esc(x.expected)+'</b></div><div class="horse-info-cell"><small>距離変更</small><b>'+esc(distTxt)+'</b></div><div class="horse-info-cell"><small>前走の前進区分</small><b>'+esc(firstThreeType(h))+'</b></div></div><div class="detail-heading">今回の位置取り診断</div><div class="pressure-grid"><div class="pressure-chip '+(q.leftHot?'danger':'safe')+'"><small>内隣圧力</small><b>'+Math.round(n(q.left)*100)+'%</b></div><div class="pressure-chip '+(q.rightHot?'danger':'safe')+'"><small>外隣圧力</small><b>'+Math.round(n(q.right)*100)+'%</b></div><div class="pressure-chip '+(q.sandwich?'danger':'')+'"><small>逃げハサミ</small><b>'+(q.sandwich?'成立警戒':'なし')+'</b></div><div class="pressure-chip"><small>判定</small><b>'+esc(pressureTxt)+'</b></div><div class="pressure-chip"><small>最初から3番手内</small><b>'+Math.round(x.early3*100)+'%</b></div><div class="pressure-chip"><small>途中から3番手内</small><b>'+Math.round(x.moved3*100)+'%</b></div><div class="pressure-chip"><small>差し上げ力</small><b>'+Math.round(x.comeFromBehind*100)+'</b></div><div class="pressure-chip"><small>下がり率</small><b>'+fade+'%</b></div></div><div class="detail-heading">今回条件への適性</div><div class="fit-grid"><div class="fit-chip"><small>距離</small><b>'+fitText('distance')+'</b></div><div class="fit-chip"><small>競馬場</small><b>'+fitText('track')+'</b></div><div class="fit-chip"><small>馬場</small><b>'+fitText('condition')+'</b></div><div class="fit-chip"><small>天候</small><b>'+fitText('weather')+'</b></div><div class="fit-chip"><small>季節</small><b>'+fitText('season')+'</b></div><div class="fit-chip"><small>相手レベル</small><b>'+fitText('level')+'</b></div></div>'+roleDetail('騎手成績',h.jockeyStats,j)+roleDetail('調教師成績',h.trainerStats,t)+'<div class="recent-list-title">近走データ（最大12走）</div>'+(recent.length?recent.map(function(rr){return'<div class="recent"><div class="recent-head"><b>'+esc(rr.date)+' '+esc(rr.track)+' '+esc(rr.distance)+'m</b><strong>'+esc(rr.finish||'—')+'着</strong></div><div>'+fmtTime(rr.timeSeconds)+'　'+esc(rr.condition||'不明')+' / '+esc(rr.weather||'不明')+'</div><div class="muted">通過 '+esc((rr.cornerPositions||[]).join('-')||'—')+'　頭数 '+esc(rr.fieldSize||'—')+(n(rr.carriedWeight)>0?'　斤量 '+esc(rr.carriedWeight)+'kg':'')+(n(rr.racePrize1)>0?'　1着賞金 '+fmtMoney(rr.racePrize1):'')+(rr.jockey?'　騎手 '+esc(rr.jockey):'')+(rr.trainer?'　調教師 '+esc(rr.trainer):'')+'</div></div>'}).join(''):'<div class="empty compact">近走データなし</div>')+'</div></details>'}).join('')+'</div></section>'}
...
function frontBattleData(p){var rows=p.rows||[],front=rows.filter(function(x){return x.goProb>=.45}).slice(),leader=front.slice().sort(function(a,b){function z(x){return x.goProb*.38+x.ten*.27+x.needLead*.20+x.front*.10-x.frontCost*.16}return z(b)-z(a)}),second=front.slice().sort(function(a,b){return (b.goProb*.26+b.stalk*.28+b.flexibility*.20+b.holdFront*.16)-(a.goProb*.26+a.stalk*.28+a.flexibility*.20+a.holdFront*.16)}),good=rows.filter(function(x){return x.expected==='好位'||x.expected==='先行'}).slice().sort(function(a,b){return (b.ability*.35+b.holdFront*.25+b.trafficTol*.20+b.goProb*.20)-(a.ability*.35+a.holdFront*.25+a.trafficTol*.20+a.goProb*.20)}),nums=front.map(function(x){return n(x.horse.horseNumber)}).sort(function(a,b){return a-b}),adj=0,i;for(i=1;i<nums.length;i++)if(nums[i]-nums[i-1]===1)adj++;var pattern=front.length<2?'単独寄り':(adj>=Math.max(1,front.length-2)?'先行列が連続':(adj===0?'先行馬が飛び飛び':'一部で先行馬が連続')),sand=front.filter(function(x){return x.sandwichRisk}),fade=front.filter(function(x){return x.fade>=.50});return{leader:leader,second:second,good:good,front:front,pattern:pattern,sand:sand,fade:fade}}
function frontBattleSection(p){var d=frontBattleData(p),hana=d.leader.slice(0,3).map(rowName).join(' / ')||'—',second=d.second.filter(function(x){return !d.leader.length||x.horse.horseNumber!==d.leader[0].horse.horseNumber}).slice(0,3).map(rowName).join(' / ')||'—',good=d.good.slice(0,3).map(rowName).join(' / ')||'—',alerts=[];if(d.sand.length)alerts.push('逃げハサミ: '+d.sand.map(rowName).join('・'));if(d.fade.length>=2)alerts.push('前候補の高下がり率: '+d.fade.map(rowName).join('・'));return'<section class="card"><div class="section-title">今回、本当に前へ行く馬</div><div class="front-battle-grid"><div class="front-battle-box"><small>ハナ候補</small><b>'+esc(hana)+'</b></div><div class="front-battle-box"><small>2番手候補</small><b>'+esc(second)+'</b></div><div class="front-battle-box"><small>好位候補</small><b>'+esc(good)+'</b></div></div><div class="front-battle-note"><b>先行列:</b> '+esc(d.pattern)+'　/　今回前候補 '+d.front.length+'頭。頭数だけで前崩れ判定はしません。</div>'+(alerts.length?'<div class="front-battle-alert">'+alerts.map(esc).join(' / ')+'</div>':'')+'</section>'}
...
function paceBoard(r,p){var hs=(r.horses||[]).slice().sort(function(a,b){return n(a.horseNumber)-n(b.horseNumber)}),stages=p.plan.stages||[],cp=courseProfile(r);return'<section class="card ai-flow-card"><div class="ai-flow-head"><span class="ai-flow-bars"><i></i><i></i><i></i></span><div class="ai-flow-copy"><div class="ai-flow-title">AI展開予想</div><div class="ai-flow-sub">脚質・枠順・テン・隣接圧力・コース形態から本線の隊列を予測</div></div><span class="ai-flow-scenario">'+esc(p.plan.scenario.code)+' '+esc(p.plan.scenario.title)+'</span></div><div class="ai-stage-tabs">'+stages.map(function(s,i){return'<button data-pace-stage="'+i+'" class="'+(i===0?'active':'')+'">'+esc(s.label)+'</button>'}).join('')+'</div><div id="pace-board" class="ai-race-visual"><div class="ai-race-meta">'+esc(r.track)+'　'+esc(r.distance)+'m　'+esc(cp.turn)+(cp.shape==='straight'?'':'回り')+'</div>'+hs.map(function(h){var ms=markSymbol(p,h.horseNumber);return'<div class="ai-race-runner" data-horse="'+esc(h.horseNumber)+'" style="left:10%;top:50%">'+(ms?'<span class="course-mark">'+ms+'</span>':'')+badge(h)+'<span class="pace-name">'+esc(h.name)+'</span></div>'}).join('')+'<div id="course-order" class="ai-race-order">隊列を準備中</div></div><div id="pace-event" class="ai-stage-event">展開イベントを準備中</div><div class="ai-race-note">上の地点を押すと、その局面の隊列へ馬が自動で移動します。外回し・被され・前の下がり・差し上げも局面ごとの位置変化に反映。</div></section>'}
