...
app = FastAPI(title="競馬展開AI", version="5.3-production-v37-auto-history")
...
<link rel="manifest" href="/manifest.webmanifest?v=37">
...
var state={date:today(),circuit:"地方",races:[],track:null,race:null,raceLoading:null,picker:false,loading:false,error:null,timer:null,anim:null,simSpeed:5,simTarget:20,simRunning:false,simPaused:false,simStopped:false,simIndex:0,simDone:0,simCounts:null,simCurrentT:0,pred:null,requestSeq:0,historyTimer:null};

function cacheKey(d){return "keiba:v37:races:"+d}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=37" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=37" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function historySearchSection(r){var hs=r.historySearch||{},cv=hs.coverage||{},months=n(hs.monthsDone),max=n(hs.maxMonths,18),progress=max?clamp(months/max*100,4,96):8,horseHtml=(r.horses||[]).map(function(h){return'<span>'+badge(h)+esc(h.name||'')+'</span>'}).join('');return'<section class="card history-search-card"><div class="history-search-head"><span class="history-spinner"></span><div><div class="history-search-title">過去走データを検索中</div><div class="history-search-sub">NAR公式の月次レースデータを過去へさかのぼって取得しています。固定値で脚質を埋めず、実データが揃ってから予想を作ります。</div></div></div><div class="history-progress"><i style="width:'+progress+'%"></i></div><div class="history-stats"><span>検索 '+months+' / '+max+'か月</span><span>履歴あり '+n(cv.horsesWithHistory)+' / '+n(cv.totalHorses,(r.horses||[]).length)+'頭</span><span>4走以上 '+n(cv.horsesWith4Plus)+'頭</span><span>取得 '+n(cv.totalRuns)+'走</span></div><div class="history-horses">'+horseHtml+'</div></section>'}
function scheduleHistoryPoll(id){if(state.historyTimer){clearTimeout(state.historyTimer);state.historyTimer=null}state.historyTimer=setTimeout(function(){fetch('/api/v1/race/'+encodeURIComponent(id)+'?refresh=1&v=37',{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('API '+res.status);return res.json()}).then(function(body){if(!state.race||String(state.race.id)!==String(id))return;state.race=body;render();if(body.historySearch&&body.historySearch.status==='running')scheduleHistoryPoll(id)}).catch(function(){if(state.race&&String(state.race.id)===String(id))scheduleHistoryPoll(id)})},1300)}
function openRace(id){if(!id||state.raceLoading)return;if(state.historyTimer){clearTimeout(state.historyTimer);state.historyTimer=null}state.raceLoading=String(id);state.race=null;state.error=null;state.picker=false;render();fetch('/api/v1/race/'+encodeURIComponent(id)+'?v=37',{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('API '+res.status);return res.json()}).then(function(body){if(String(state.raceLoading)!==String(id))return;state.raceLoading=null;state.race=body;render();if(body.historySearch&&body.historySearch.status==='running')scheduleHistoryPoll(id)}).catch(function(e){if(String(state.raceLoading)!==String(id))return;state.raceLoading=null;state.error='レース詳細の取得に失敗しました';render()})}
function reloadCurrent(){if(state.race&&state.race.id){var id=state.race.id;state.race=null;openRace(id)}else load()}
function renderRace(){var r=state.race,hs=r.historySearch||{};if(hs.status==='running'){state.pred=null;return'<div class="shell">'+header(r.track+' '+r.raceNumber+'R',true,(r.title||'')+'｜'+r.distance+'m・'+r.condition)+'<main class="main"><section class="card race-title-card"><div class="row between"><div><h2>'+esc(r.title||"")+'</h2><div class="muted">'+esc(r.date)+' '+timeHtml(r)+'</div></div><span class="pill">'+esc(r.circuit)+'</span></div></section>'+historySearchSection(r)+'</main></div>'}var p=predict(r);state.pred=p;var sourceNote=hs.status==='done'?'<div class="data-source-note">過去走検索：'+n(hs.monthsDone)+'か月確認 / '+n((hs.coverage||{}).totalRuns)+'走取得</div>':(hs.status==='error'?'<div class="notice">過去走の追加取得に失敗。取得済みデータだけで表示します。</div>':'');return'<div class="shell">'+header(r.track+' '+r.raceNumber+'R',true,(r.title||'')+'｜'+r.distance+'m・'+r.condition)+'<main class="main">'+renderResult(r)+renderResultGap(r,p)+renderActualFlow(r)+'<section class="card race-title-card"><div class="row between"><div><h2>'+esc(r.title||"")+'</h2><div class="muted">'+esc(r.date)+' '+timeHtml(r)+'</div></div><span class="pill">'+esc(r.circuit)+'</span></div><div class="metrics"><div class="metric"><b>'+esc(r.distance)+'m</b><span>距離</span></div><div class="metric"><b>'+esc(r.condition||'不明')+'</b><span>馬場</span></div><div class="metric"><b>'+esc(r.weather||'不明')+'</b><span>天候</span></div><div class="metric"><b>'+season(r.date)+'</b><span>季節</span></div><div class="metric"><b>'+esc((r.horses||[]).length)+'頭</b><span>頭数</span></div></div><div class="data-depth"><span>近走 最大12走</span><span>NAR公式 過去走自動検索</span><span>0走は推定値を表示しない</span></div>'+sourceNote+'</section>'+runnerStyleSection(r,p)+scenarioProbabilitySection(p)+frontBattleSection(p)+paceBoard(r,p)+'<section class="card"><div class="section-title">予想印</div><div class="marks">'+p.marks.map(function(m){return'<div class="mark"><span class="mark-symbol">'+m[0]+'</span>'+badge(m[1])+'<strong>'+esc(m[1].name)+'</strong></div>'}).join("")+'</div></section></main></div>'}
...
def _history_worker(months_back: int | None = None):
    ...
    time.sleep(float(os.getenv("HISTORY_START_DELAY_SEC", "0.5")))
    sync = NarSync()
    today = datetime.now().date()
...
def _history_counts(horse_names: list[str], cutoff: str) -> dict:
    ...
    if not names:
        return {"totalHorses":0,"horsesWithHistory":0,"horsesWith4Plus":0,"horsesWith6Plus":0,"totalRuns":0,"counts":{}}
    store = NarStore()
    try:
...
def _history_is_enough(cov: dict, months_done: int) -> bool:
    ...
    if total <= 0:
        return True
    four = int(cov.get("horsesWith4Plus") or 0)
    anyh = int(cov.get("horsesWithHistory") or 0)
    # Most runners need several actual starts for a useful style map. Newcomers should not
    # force a two-year crawl forever, so after six months allow a small zero-history minority.
    if four >= max(1, int((total * 0.80) + 0.999)):
        return True
    if months_done >= 6 and anyh >= max(1, int((total * 0.88) + 0.999)) and four >= max(1, int((total * 0.68) + 0.999)):
        return True
    return False

def _start_race_history_search(race_id: str, iso_date: str, horse_names: list[str], force: bool = False) -> dict:
    max_months = max(6, min(24, int(os.getenv("NAR_ON_DEMAND_HISTORY_MONTHS", "18"))))
    ...
    def worker():
        ...
        try:
            for y, m in _months_from_race_date(iso_date, max_months):
                try:
                    with _nar_sync_io_lock:
                        sync.sync_month(y, m, force=False)
                except Exception as exc:
                    errors.append(f"{y:04d}-{m:02d}:{exc}")
                months_done += 1
                cov_now = _history_counts(horse_names, iso_date)
                with _race_history_lock:
                    _race_history_jobs[race_id] = {"status":"running","monthsDone":months_done,"maxMonths":max_months,"coverage":cov_now,"error":" | ".join(errors[-3:]),"source":"NAR公式月次レースデータ"}
                if _history_is_enough(cov_now, months_done):
                    break
                time.sleep(float(os.getenv("NAR_ON_DEMAND_GAP_SEC", "0.15")))
            cov_now = _history_counts(horse_names, iso_date)
            status = "done" if cov_now.get("horsesWithHistory",0) > 0 else ("error" if errors else "done")
            with _race_history_lock:
                _race_history_jobs[race_id] = {"status":status,"monthsDone":months_done,"maxMonths":max_months,"coverage":cov_now,"error":" | ".join(errors[-5:]),"source":"NAR公式月次レースデータ"}
            try:
...
def race_detail(race_id: str, refresh: int = Query(0)):
    now=time.time()
    if not refresh:
        with _detail_cache_lock:
    ...
    if race_id.startswith("nar-"):
        ...
        # Rebuild after each background-import step so recentRaces/stats are fresh.
        ...
        detail["historySearch"]={"status":"done","monthsDone":0,"maxMonths":0,"coverage":{"totalHorses":len(detail.get("horses",[])),"totalRuns":sum(len(h.get("recentRaces") or []) for h in detail.get("horses",[]))},"error":"","source":detail.get("source") or "中央フィード"}
    with _detail_cache_lock:
