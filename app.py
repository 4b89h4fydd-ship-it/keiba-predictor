...
app = FastAPI(title="競馬展開AI", version="8.5-production-v73-central-odds-results-scenarios")
...
<link rel="manifest" href="/manifest-v73.webmanifest">
<link rel="stylesheet" href="/styles-v73.css">
<title>競馬展開AI</title>
</head>
<body>
<div id="app"><div class="boot">競馬展開AIを起動中…</div></div>
<script src="/app-v73.js"></script>
</body>
</html>"""
...
function cacheKey(d,c){return "keiba:v73:races:"+d+":"+(c||state.circuit||"")}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=72" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence · v73</div></div><button class="hero-refresh '+(state.loading?'is-loading':'')+'" data-action="reload"><span class="refresh-icon">↻</span><small>'+(state.loading?'更新中':'更新')+'</small></button></div><main class="home-main">'+
...
function scenarioProbabilitySection(p){var top=p.scenarios.slice().sort(function(a,b){return b.prob-a.prob})[0],active=(state.scenarioCode&&p.plans&&p.plans[state.scenarioCode])?state.scenarioCode:top.code,front=p.rows.filter(function(x){return x.goProb>=.52}),sand=front.filter(function(x){return x.sandwichRisk}),highFade=front.filter(function(x){return x.fade>=.50}),occ=p.occ||{rate:0,early:[],moved:[]},warn=[];if(sand.length)warn.push('逃げハサミ警戒 馬番 '+horseNoList(sand));if(highFade.length>=2)warn.push('高下がり率の前候補 馬番 '+horseNoList(highFade)+' → 差し馬を再検査');if(!warn.length)warn.push('単純な先行頭数ではなく、枠順・隣接圧力・残り性能から展開判定');var activeScenario=p.scenarios.filter(function(s){return s.code===active})[0]||top;return'<section class="card pace-card"><div class="section-title">展開シナリオ</div><div class="scenario-prob-grid">'+p.scenarios.map(function(s){return'<button type="button" data-scenario-code="'+esc(s.code)+'" class="scenario-prob-card '+(s.code===active?'active':'')+'"><small>'+s.code+'</small><b>'+esc(s.title)+'</b><strong>'+Math.round(s.prob*100)+'%</strong><span>候補馬番 '+horseNoList(s.horses)+'</span></button>'}).join('')+'</div><div class="scenario-main-note"><span>表示中シナリオ</span><b>'+esc(activeScenario.code)+' '+esc(activeScenario.title)+'　'+Math.round(activeScenario.prob*100)+'%　候補馬番 '+horseNoList(activeScenario.horses)+'</b></div><div class="scenario-diagnostics"><div class="scenario-diagnostic"><small>先行馬占有率</small><b>'+Math.round(occ.rate*100)+'%</b></div><div class="scenario-diagnostic"><small>最初から前</small><b>馬番 '+horseNoList(occ.early)+'</b></div><div class="scenario-diagnostic"><small>途中から上昇</small><b>馬番 '+horseNoList(occ.moved)+'</b></div><div class="scenario-diagnostic"><small>今回前候補</small><b>馬番 '+horseNoList(front)+'</b></div></div><div class="scenario-warning">'+warn.map(esc).join(' / ')+'</div></section>'}
...
@app.get("/build")
def build_info():
    return {"build":"v73","appVersion":"8.5-production-v73-central-odds-results-scenarios","recentRuns":5,"allCentralVenues":True,"deterministicCentralDetailId":True,"centralResults":True,"centralLiveOddsApi":True,"scenarioABCSelectable":True,"scenarioHorseNumbers":True,"stickyVenueHeader":True,"circuitSpecificFetch":True,"manualCollect":True,"builtInSources":["JRA公式","NAR公式","netkeiba"],"smartRcDirectLink":False,"multiSource":True,"nonBlocking":True,"grade":"S/A/B/C + ◎○▲☆△注"}
...
@app.get("/manifest-v73.webmanifest")
def manifest():
