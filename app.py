...
app = FastAPI(title="競馬展開AI", version="8.61-production-v86-racedb-edgeback")
...
<script src="/app-v86-fix1.js"></script>
</body>
</html>"""
...
function openRace(id,keepStack,skipHistory){if(!id||state.raceLoading)return;var seq=++state.detailSeq;if(!keepStack){state.raceStack=[];state.raceReturnPicker=state.picker}state.horseModalNo=null;if(state.historyTimer){clearTimeout(state.historyTimer);state.historyTimer=null}state.error=null;state.picker=false;state.scenarioCode=null;state.paceStage=0;var cached=loadDetailCache(id),hasCached=!!(cached&&((cached.horses||[]).length||isFinal(cached)));if(hasCached){state.raceLoading=null;state.race=cached;render()}else{state.raceLoading=String(id);state.race=null;render()}fetch('/api/v1/race/'+encodeURIComponent(id)+'?prepared=1&v=84'+(skipHistory?'&history=0':''),{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('API '+res.status);return res.json()}).then(function(body){if(seq!==state.detailSeq)return;state.raceLoading=null;state.race=body;saveDetailCache(id,body);render();if((!skipHistory&&((body.historySearch&&body.historySearch.status==='running')||(body.preparedMeta&&body.preparedMeta.prepared)))||(body.enrichmentSearch&&body.enrichmentSearch.status==='running'))scheduleHistoryPoll(id)}).catch(function(){if(seq!==state.detailSeq)return;if(hasCached){state.raceLoading=null;state.race=cached;render();return}if(String(state.raceLoading)!==String(id))return;state.raceLoading=null;state.error='レース詳細の取得に失敗しました';if(state.raceStack.length)state.race=state.raceStack.pop();render()})}
function openPastRace(id){if(!id)return;if(state.race)state.raceStack.push(state.race);openRace(id,true,true)}
function reloadCurrent(){if(state.loading)return;if(state.race&&state.race.id){var seq=++state.detailSeq,id=state.race.id,isPast=state.raceStack.length>0;if(state.historyTimer){clearTimeout(state.historyTimer);state.historyTimer=null}state.raceLoading=String(id);state.race=null;render();fetch('/api/v1/race/'+encodeURIComponent(id)+'?refresh=1&v=84'+(isPast?'&history=0':''),{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('API '+res.status);return res.json()}).then(function(body){if(seq!==state.detailSeq)return;state.raceLoading=null;state.race=body;saveDetailCache(id,body);render();if((body.historySearch&&body.historySearch.status==='running')||(body.enrichmentSearch&&body.enrichmentSearch.status==='running'))scheduleHistoryPoll(id)}).catch(function(){if(seq!==state.detailSeq)return;state.raceLoading=null;state.error='更新に失敗しました';render()});return}try{localStorage.removeItem(cacheKey(state.date,state.circuit))}catch(e){}state.races=[];load(true)}
...
function goBack(){
    ...
    render();
    if(state.race&&((state.race.historySearch||{}).status==='running'||(state.race.enrichmentSearch||{}).status==='running'))scheduleHistoryPoll(state.race.id);
}
...
installEdgeBack();clearOldPwa();render();setTimeout(load,0);
})();
"""
...
class PreparedRaceStore:
    def __init__(self, path: Path = PREPARED_DB_PATH):
        ...
        try:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_prepared_date ON prepared_races(race_date, circuit)")
            conn.commit()
        finally:
            conn.close()
    ...
    def get(self, race_id: str) -> tuple[dict | None, int]:
        ...
        conn.row_factory = sqlite3.Row
        try:
            row = conn.execute("SELECT updated_at,payload FROM prepared_races WHERE race_id=?", (race_id,)).fetchone()
            if not row:
                return None, 0
            try:
    ...
    def put(self, detail: dict) -> None:
        ...
        try:
            conn.execute(
                ...
                """,
                (race_id, str(detail.get("date") or ""), str(detail.get("circuit") or ""), now, payload),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def status(self, race_date: str = "", circuit: str = "") -> dict:
        ...
        try:
            where=[]; args=[]
            if race_date:
                where.append("race_date=?"); args.append(race_date)
            if circuit:
                where.append("circuit=?"); args.append(circuit)
...
def _pc_style_metrics(horse: dict, race: dict) -> dict:
    rs=list(horse.get("recentRaces") or [])[:5]
...
def _pc_fit_metrics(horse: dict, race: dict) -> dict:
    rs=list(horse.get("recentRaces") or [])[:5]; target=int(race.get("distance") or 0); target_season=_pc_season(str(race.get("date") or ""))
...
def _precompute_detail_metrics(detail: dict) -> dict:
    ...
    for horse in detail.get("horses",[]) or []:
        try:
            horse["precomputedMetrics"]={"style":_pc_style_metrics(horse,detail),"fit":_pc_fit_metrics(horse,detail)}
        except Exception:
...
def _prepare_race_snapshot(race_id: str, force: bool = False) -> dict | None:
    if not force:
        hit=_prepared_get_fresh(race_id)
        if hit:return hit
...
def _ensure_detail_background_jobs(race_id: str, detail: dict) -> None:
    ...
    try:
        if race_id.startswith("nar-"):
            ...
            if _race_history_status(race_id) is None and resolved<total:_start_central_history_search(race_id,detail.get("date") or _today_iso())
    except Exception as exc:
...
def race_detail(race_id: str, refresh: int = Query(0), history: int = Query(1), prepared: int = Query(1)):
    ...
    if prepared and not refresh:
        fast=_prepared_get_fresh(race_id)
        if fast:
