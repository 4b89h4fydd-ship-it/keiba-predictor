...
app = FastAPI(title="競馬展開AI", version="4.3-production-v24-unified-detail")
...
function makeMarks(rows,suit){var a=rows.slice().sort(function(x,y){var sx=suit[n(x.horse.horseNumber)],sy=suit[n(y.horse.horseNumber)];return sy.rankScore-sx.rankScore}),syms=["◎","○","▲","☆","△","注"],out=[],i;for(i=0;i<Math.min(syms.length,a.length);i++)out.push([syms[i],a[i].horse]);return out}
function predict(r){var rows=buildRows(r),occ=earlyOcc(r),sc=scenarioModel(r,rows),suit=suitability(rows,sc),marks=makeMarks(rows,suit),start=startOrder(rows),corner=cornerScores(r,rows,sc,suit),straight=rows.slice().sort(function(a,b){return suit[n(b.horse.horseNumber)].rankScore-suit[n(a.horse.horseNumber)].rankScore});var pressure=pressureInfo(rows);for(var i=0;i<sc.length;i++){var code=sc[i].code,candidates=rows.slice().sort(function(a,b){return scenarioSuit(b,code,pressure)-scenarioSuit(a,code,pressure)});sc[i].horses=candidates.slice(0,3).map(function(x){return x.horse})}var cov=mean(rows.map(function(x){return x.coverage}));return{rows:rows,occ:occ,scenarios:sc,marks:marks,plan:{start:start,corner:corner,straight:straight},suit:suit,coverage:cov,pressure:pressureInfo(rows)}}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=25" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=25" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function setPaceStage(st){state.stage=st;var r=state.race;if(!r)return;var p=predict(r),order=orderAt(p,st),board=document.getElementById('pace-board'),path=document.getElementById('course-path');if(!board||!path)return;var labels=['スタート','4コーナー','直線'],name=document.getElementById('pace-stage-name');if(name)name.textContent=labels[st];var btns=document.querySelectorAll('[data-stage]'),i;for(i=0;i<btns.length;i++)btns[i].className=n(btns[i].getAttribute('data-stage'))===st?'active':'';var cp=courseProfile(r),total=path.getTotalLength(),base=courseStageFrac(r,st),spread=st===0?.007:(st===1?.017:.024),max=Math.max(1,order.length-1),rect=board.getBoundingClientRect(),viewW=200,viewH=180;for(i=0;i<order.length;i++){var row=order[i],chip=board.querySelector('[data-horse="'+row.horse.horseNumber+'"]');if(!chip)continue;var behind=i/max*spread,frac=normFrac(base-cp.dir*behind),pt=path.getPointAtLength(frac*total),x=pt.x/viewW*rect.width,y=pt.y/viewH*rect.height,lane=((n(row.horse.horseNumber)%3)-1)*4;chip.style.left=x+'px';chip.style.top=(y+lane)+'px'}var startDot=document.getElementById('course-start-dot'),sl=document.getElementById('course-start-label');if(startDot){var sf=courseStageFrac(r,0),sp=path.getPointAtLength(sf*total);startDot.setAttribute('cx',sp.x);startDot.setAttribute('cy',sp.y);if(sl){sl.setAttribute('x',Math.min(176,sp.x+4));sl.setAttribute('y',Math.max(10,sp.y-5))}}var orderBox=document.getElementById('course-order');if(orderBox)orderBox.innerHTML='<b>'+labels[st]+'</b>　'+order.slice(0,Math.min(8,order.length)).map(function(x){return esc(x.horse.horseNumber)+' '+esc(x.horse.name)}).join(' → ')}
...
def health():
    ...
    return {
        "status":"ok", "mode":"production-v23-course-ui", "historyStarted":_history_started,
