...
app = FastAPI(title="競馬展開AI", version="4.8-production-v27-deep-sim-controls")
...
var state={date:today(),circuit:"地方",races:[],track:null,race:null,picker:false,loading:false,error:null,timer:null,anim:null,simSpeed:5,simTarget:20,simRunning:false,simPaused:false,simStopped:false,simIndex:0,simDone:0,simCounts:null,simCurrentT:0,pred:null};
...
'<section class="home-card home-ai-card"><div class="home-ai-top"><span class="home-ai-bars"><i></i><i></i><i></i></span><div class="home-ai-copy"><div class="home-ai-title">AI展開シミュレーション</div><div class="home-ai-sub">50通りの展開を走らせ、1〜3着を総合判定します</div></div>'+aiButtons+'</div>'+miniPacePreview(nx)+'</section>'+
'</main></div>'}
function renderPicker(){var a=state.races.filter(function(r){return r.circuit===state.circuit&&!isFinal(r)});a.sort(function(x,y){return mins(x.startTime)-mins(y.startTime)});return'<div class="shell">'+header("シミュレーションを選択",true,state.date+'・'+state.circuit)+'<main class="main"><section class="card"><div class="picker-list">'+(a.length?a.map(function(r){return'<button class="picker-item" data-race="'+esc(r.id)+'"><span>'+esc(r.track)+' '+esc(r.raceNumber)+'R　'+esc(r.title||"")+'</span><strong>'+timeHtml(r)+'</strong></button>'}).join(""):'<div class="empty">未確定レースはありません</div>')+'</div></section></main></div>'}
...
def health():
    ...
    return {
        "status":"ok", "mode":"production-v27-deep-sim", "historyStarted":_history_started,
...
def races(date: str = Query(...)):
    ...
    def _bg_nar_sync():
        try:
            ...
            sync = NarSync()
            target_back = max(4, min(12, int(os.getenv("NAR_TARGET_HISTORY_MONTHS", "9"))))
            jobs = [(target.year, target.month)] + list(iter_months_back(target, target_back))
            for y, m in jobs:
