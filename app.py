...
function cornerScores(r,rows,sc,suit){var top=sc.slice().sort(function(a,b){return b.prob-a.prob})[0].code,out=[],i,x,s,z;for(i=0;i<rows.length;i++){x=rows[i];z=suit[n(x.horse.horseNumber)]||{overall:0};s=x.forward*.38+x.ability*.18+z.overall*.30+x.move*.16-x.fade*.18;if(top==="A")s+=x.forward*.20-x.fade*.10;if(top==="C")s+=x.move*.24+x.mid*.09+x.close*.13-x.forward*.03;out.push({row:x,s:s})}return out.sort(function(a,b){return b.s-a.s}).map(function(z){return z.row})}
...
function predict(r){var rows=buildRows(r),occ=earlyOcc(r),sc=scenarioModel(r,rows),suit=suitability(rows,sc),marks=makeMarks(rows,suit),start=startOrder(rows),corner=cornerScores(r,rows,sc,suit),straight=rows.slice().sort(function(a,b){return suit[n(b.horse.horseNumber)].overall-suit[n(a.horse.horseNumber)].overall});for(var i=0;i<sc.length;i++){var code=sc[i].code,candidates=rows.slice().sort(function(a,b){function ss(x){var z=suit[n(x.horse.horseNumber)];if(code==="A")return z.overall+x.forward*.22-x.fade*.12;if(code==="C")return z.overall+x.move*.18+x.close*.12;return z.overall}return ss(b)-ss(a)});sc[i].horses=candidates.slice(0,3).map(function(x){return x.horse})}return{rows:rows,occ:occ,scenarios:sc,marks:marks,plan:{start:start,corner:corner,straight:straight},suit:suit}}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=24" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
return '<div class="home-shell"><div class="home-hero"><img class="hero-horse" src="/hero-horse.webp?v=24" alt=""><div class="brand-wrap"><div class="brand-main">競馬展開<span class="ai">AI</span></div><div class="brand-sub">Race Intelligence</div></div><button class="hero-refresh" data-action="reload"><span class="refresh-icon">↻</span><small>更新</small></button></div><main class="home-main">'+
...
function renderPaceProbability(p){var q=paceProbability(p);return'<section class="card pace-prob-card"><div class="section-title">展開予想の確率</div><div class="pace-prob"><b>'+Math.round(q*100)+'%</b><div class="pace-prob-track"><span style="width:'+Math.round(q*100)+'%"></span></div></div><div class="muted">表示している本線隊列の推定発生率</div></section>'}
