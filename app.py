...
app = FastAPI(title="競馬展開AI", version="2.0-live-local")
...
class NarSync:
    ...
    def _download(self, url: str, destination: Path) -> bytes:
        request = urllib.request.Request(
            url,
            headers={
                "User-Agent": "KeibaPredictor/0.5 (+personal-use data client)",
    ...
    def sync_month(self, year: int, month: int, force: bool = False) -> dict:
        ...
        if not force and self.store.synced_at(cache_key):
            return {"status": "cached", "cacheKey": cache_key}
...
def iter_months_back(target: date, count: int) -> Iterator[tuple[int, int]]:
...
def races(date: str = Query(...)):
    ...
    live_local = []
    # Today's local card comes from NAR official daily ZIP.
    if date == _today_iso():
        try:
            NarSync().sync_daily(force=False)
            ensure_history_async()
            live_local = NarStore().races_json(date)
        except Exception as exc:
            print(f"NAR daily sync failed: {exc}")
    else:
        # Serve cached historical local races if available.
        try:
            live_local = NarStore().races_json(date)
        except Exception:
            live_local = []
    ...
    local_samples = [r for r in sample_rows if r.get("circuit") == "地方"]
    # If live NAR data exists, never mix demo local races into the same day.
    return live_local + central_samples + ([] if live_local else local_samples)
