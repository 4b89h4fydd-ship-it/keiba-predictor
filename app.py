...
app = FastAPI(title="競馬展開AI", version="4.6-production-v26-sim50")
...
var COURSE={
"帯広":{lap:200,straight:200,dir:1,shape:"straight",turn:"直線"},
"門別":{lap:1600,straight:330,dir:-1,shape:"wide",turn:"右"},
"盛岡":{lap:1600,straight:300,dir:1,shape:"wide",turn:"左"},
"水沢":{lap:1200,straight:245,dir:-1,shape:"compact",turn:"右"},
"浦和":{lap:1200,straight:220,dir:1,shape:"compact",turn:"左"},
"船橋":{lap:1400,straight:308,dir:1,shape:"boxy",turn:"左"},
"大井":{lap:1600,straight:386,dir:-1,shape:"wide",turn:"右"},
"川崎":{lap:1200,straight:300,dir:1,shape:"compact",turn:"左"},
"金沢":{lap:1200,straight:236,dir:-1,shape:"pocket",turn:"右"},
"笠松":{lap:1100,straight:201,dir:-1,shape:"compact",turn:"右"},
"名古屋":{lap:1180,straight:240,dir:-1,shape:"spiral",turn:"右"},
"園田":{lap:1051,straight:213,dir:-1,shape:"compact",turn:"右"},
"姫路":{lap:1200,straight:230,dir:-1,shape:"boxy",turn:"右"},
"高知":{lap:1100,straight:200,dir:-1,shape:"egg",turn:"右"},
"佐賀":{lap:1100,straight:200,dir:-1,shape:"compact",turn:"右"},
"札幌":{lap:1641,straight:266,dir:-1,shape:"round",turn:"右"},
"函館":{lap:1627,straight:262,dir:-1,shape:"compact",turn:"右"},
"福島":{lap:1600,straight:292,dir:-1,shape:"boxy",turn:"右"},
"新潟":{lap:2223,straight:659,dir:1,shape:"long",turn:"左"},
"東京":{lap:2084,straight:526,dir:1,shape:"long",turn:"左"},
"中山":{lap:1667,straight:310,dir:-1,shape:"boxy",turn:"右"},
"中京":{lap:1706,straight:413,dir:1,shape:"wide",turn:"左"},
"京都":{lap:1783,straight:328,dir:-1,shape:"wide",turn:"右"},
"阪神":{lap:1689,straight:357,dir:-1,shape:"wide",turn:"右"},
"小倉":{lap:1615,straight:293,dir:-1,shape:"compact",turn:"右"}
};
function courseProfile(r){return COURSE[r.track]||{lap:1400,straight:300,dir:-1,shape:"wide",turn:"右"}}
...
var state={date:today(),circuit:"地方",races:[],track:null,race:null,picker:false,loading:false,error:null,timer:null,anim:null,simSpeed:5,simRunning:false,simIndex:0,simDone:0,simCounts:null,pred:null};
...
function styleRates(h){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),c=[0,0,0,0],den=0,i,p,x,ww;for(i=0;i<rs.length;i++){p=rs[i].cornerPositions||[];x=n(p[0],0);if(!x)continue;ww=w[i];den+=ww;if(x===1)c[0]+=ww;else if(x<=4)c[1]+=ww;else if(x<=7)c[2]+=ww;else c[3]+=ww}if(!den)return{front:.05,stalk:.25,mid:.45,close:.25};return{front:c[0]/den,stalk:c[1]/den,mid:c[2]/den,close:c[3]/den}}
function fadeRate(h){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),e=0,f=0,i,p,first,last,fin,ww;for(i=0;i<rs.length;i++){p=rs[i].cornerPositions||[];first=n(p[0]);if(!first||first>4)continue;ww=w[i];e+=ww;last=n(p[p.length-1]);fin=n(rs[i].finish);if((last&&last>=first+2)||(fin&&fin>=first+3))f+=ww}return e?f/e:.25}
function moveRate(h){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),e=0,g=0,i,p,a,b,ww;for(i=0;i<rs.length;i++){p=rs[i].cornerPositions||[];if(p.length<2)continue;a=n(p[0]);b=n(p[p.length-1]);if(!a||!b)continue;ww=w[i];e+=ww;if(b<=a-2)g+=ww}return e?g/e:.2}
function holdRate(h){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),e=0,g=0,i,p,a,b,ww;for(i=0;i<rs.length;i++){p=rs[i].cornerPositions||[];a=n(p[0]);b=n(p[p.length-1]);if(!a||a>4)continue;ww=w[i];e+=ww;if((b&&b<=4)||n(rs[i].finish)<=4)g+=ww}return e?g/e:.5}
function yieldFlex(h){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),e=0,g=0,i,p,a,fin,ww;for(i=0;i<rs.length;i++){p=rs[i].cornerPositions||[];a=n(p[0]);fin=n(rs[i].finish);if(!a||a===1||a>5)continue;ww=w[i];e+=ww;if(fin>0&&fin<=4)g+=ww}return e?g/e:.45}
...
function recentFinishScore(h){var rs=(h.recentRaces||[]).slice(0,6),w=recencyWeights(rs.length),v=[],i;for(i=0;i<rs.length;i++)v.push(runFinishQuality(rs[i]));return weightedRate(v,w,.45)}
function speedRaw(h,dist){var rs=(h.recentRaces||[]).slice(0,8),w=recencyWeights(rs.length),vals=[],ws=[],i,x;for(i=0;i<rs.length;i++){x=runTimeIndex(rs[i],dist);if(x!=null){vals.push(x);ws.push(w[i])}}if(!vals.length)return 0;var best=Math.max.apply(null,vals),avg=weightedRate(vals,ws,0);return best*.55+avg*.45}
...
function buildRows(r){var hs=r.horses||[],tmp=[],speeds=[],prizes=[],i,h,rt,pt,sr,fit;var ct=courseTraits(r);for(i=0;i<hs.length;i++){h=hs[i];rt=styleRates(h);pt=stylePoint(rt);sr=speedRaw(h,n(r.distance));fit=conditionFit(h,r);tmp.push({horse:h,front:rt.front,stalk:rt.stalk,mid:rt.mid,close:rt.close,fade:fadeRate(h),move:moveRate(h),hold:holdRate(h),yieldFlex:yieldFlex(h),score:pt,expected:styleName(pt),speedRaw:sr,fit:fit});speeds.push(sr);prizes.push(n(h.prizeMoneyAtRace))}
for(i=0;i<tmp.length;i++){var x=tmp[i],hh=x.horse,rf=recentFirst(hh),rd=recentDistance(hh),distChange=rd?rd-n(r.distance):0,shorten=distChange>=150?1:0,lengthen=distChange<=-150?1:0,no=n(hh.horseNumber),field=Math.max(1,hs.length),draw=(no-1)/Math.max(1,field-1),outer=draw>.70?1:0,edge=no===field?1:0,inner=draw<.28?1:0;var recentEarly=(rf<99?clamp((8-rf)/7,0,1):.5),jp=hh.jockeyProfile||{},tp=hh.trainerProfile||{};var jockeyFront=n(jp.early3Rate,0),leadHabit=n(jp.leaderRate,0);var needLead=clamp((x.front*1.10-x.stalk*.22)+Math.max(0,x.front-.28)*.35,0,1);var flexibility=clamp(x.yieldFlex*.62+x.stalk*.30+x.mid*.08,0,1);var shortenBoost=shorten*(x.front*.22+x.stalk*.10);var lengthenBoost=lengthen*(x.stalk*.08+x.mid*.05);var drawAdj=inner*ct.turnLoad*.07+edge*(1-ct.outerLoad)*.05-outer*ct.outerLoad*(.16-.08*flexibility);var goScore=x.front*.40+x.stalk*.25+recentEarly*.12+jockeyFront*.08+leadHabit*.04+shortenBoost+lengthenBoost+drawAdj;goScore=clamp(goScore,0,1.35);var jf=hh.jockeyProfile?roleProfileScore(hh.jockeyProfile):genericRoleScore(hh.jockeyStats),tf=hh.trainerProfile?roleProfileScore(hh.trainerProfile):genericRoleScore(hh.trainerStats);var trackFit=confidenceBlend(x.fit.track,x.fit.counts.track),distFit=confidenceBlend(x.fit.distance,x.fit.counts.distance),condFit=confidenceBlend(x.fit.condition,x.fit.counts.condition),weatherFit=confidenceBlend(x.fit.weather,x.fit.counts.weather),seasonFit=confidenceBlend(x.fit.season,x.fit.counts.season),levelFit=confidenceBlend(x.fit.level,x.fit.counts.level);var speed=normalize(speeds,x.speedRaw),prize=normalize(prizes,n(hh.prizeMoneyAtRace));var baseAbility=recentFinishScore(hh)*.21+speed*.17+distFit*.11+trackFit*.09+condFit*.07+levelFit*.09+prize*.07+jf*.07+tf*.04+seasonFit*.025+weatherFit*.015+weightScore(hh)*.04+ageSexScore(hh,r)*.03;var dataN=Math.min(8,(hh.recentRaces||[]).length),coverage=clamp(dataN/5,0,1)*.65+clamp((x.fit.counts.distance+x.fit.counts.track)/4,0,1)*.20+clamp(n(jp.starts)/30,0,1)*.15;x.forward=goScore;x.goProb=clamp(goScore*.80+x.front*.18,0,1);x.needLead=needLead;x.flexibility=flexibility;x.ability=clamp(baseAbility,0,1);x.coverage=coverage;x.draw=draw;x.outer=outer;x.edge=edge;x.inner=inner;x.shorten=shorten;x.lengthen=lengthen;x.course=ct;x.jockeyFront=jockeyFront}
return tmp}
function rowByNo(rows,no){var i;for(i=0;i<rows.length;i++)if(n(rows[i].horse.horseNumber)===n(no))return rows[i];return null}
function pressureInfo(rows){var sorted=rows.slice().sort(function(a,b){return n(a.horse.horseNumber)-n(b.horse.horseNumber)}),p={},i,x,l,r,lp,rp;for(i=0;i<sorted.length;i++){x=sorted[i];l=i?sorted[i-1]:null;r=i<sorted.length-1?sorted[i+1]:null;lp=l?l.goProb*(.55+.45*l.needLead):0;rp=r?r.goProb*(.55+.45*r.needLead):0;var leftHot=lp>.48,rightHot=rp>.48,sandwich=leftHot&&rightHot?1:0,adj=(leftHot?1:0)+(rightHot?1:0),conflict=(lp+rp)*(.55+.45*x.needLead)*(1-.45*x.flexibility);var freeOuter=x.edge&&adj===0?.12:0;p[n(x.horse.horseNumber)]={adj:adj,sandwich:sandwich,conflict:clamp(conflict,0,1.5),freeOuter:freeOuter,left:lp,right:rp}}
return p}
function scenarioModel(r,rows){var p=pressureInfo(rows),front=rows.slice().sort(function(a,b){return b.goProb-a.goProb}),active=front.filter(function(x){return x.goProb>.52}),top=front.slice(0,Math.min(4,front.length)),fade=mean(top.map(function(x){return x.fade})),hold=mean(top.map(function(x){return x.hold})),conf=mean(top.map(function(x){return (p[n(x.horse.horseNumber)]||{}).conflict||0})),sand=mean(top.map(function(x){return (p[n(x.horse.horseNumber)]||{}).sandwich||0})),needConflict=0,i,j;for(i=0;i<active.length;i++)for(j=i+1;j<active.length;j++){if(Math.abs(n(active[i].horse.horseNumber)-n(active[j].horse.horseNumber))<=2)needConflict+=active[i].needLead*active[j].needLead*.06}var clear=front.length>1?clamp((front[0].goProb-front[1].goProb)*1.5+front[0].flexibility*.10,0,.35):.2;var ct=rows.length?rows[0].course:courseTraits(r),A=.30+ct.frontBias*.22+clear+hold*.09-fade*.07-conf*.05-sand*.04,B=.34,C=.22;C=.18+(1-ct.frontBias)*.14+fade*.14+conf*.10+sand*.08+needConflict+ct.moveRoom*.06;B=.32+Math.max(0,.22-Math.abs(A-C))*.25;A=clamp(A,.16,.66);C=clamp(C,.12,.56);B=clamp(B,.20,.48);var sum=A+B+C;A/=sum;B/=sum;C/=sum;return[{code:"A",title:"前残り",prob:A},{code:"B",title:"平均",prob:B},{code:"C",title:"差し届く",prob:C}]}
...
function scenarioSuit(x,code,pressure){var q=pressure[n(x.horse.horseNumber)]||{},earlyCost=q.conflict*x.goProb*(.45+.40*x.needLead)*(1-.30*x.flexibility),frontStay=x.goProb*.30+x.hold*.24+(1-x.fade)*.19+x.ability*.21-earlyCost*.12,balanced=x.ability*.42+x.hold*.13+x.move*.12+(1-x.fade)*.10+x.goProb*.08,close=x.ability*.34+x.move*.25+x.mid*.10+x.close*.14+x.course.moveRoom*.08-x.goProb*.03;if(code==="A")return clamp(frontStay,0,1.25);if(code==="C")return clamp(close,0,1.25);return clamp(balanced,0,1.25)}
...
function stagePack(rows,scoreMap,kind){var a=rows.slice().sort(function(x,y){return scoreMap[n(y.horse.horseNumber)]-scoreMap[n(x.horse.horseNumber)]||n(x.horse.horseNumber)-n(y.horse.horseNumber)}),lead=a.length?scoreMap[n(a[0].horse.horseNumber)]:0,out=[],i,x,no,diff,baseGap;for(i=0;i<a.length;i++){x=a[i];no=n(x.horse.horseNumber);diff=Math.max(0,lead-scoreMap[no]);baseGap=(kind===0?.010:(kind===1?.014:.018))*i+diff*(kind===0?.035:(kind===1?.050:.065));out.push({no:no,gap:clamp(baseGap,0,.30),lane:clamp(Math.round((x.draw-.5)*4)+(kind===1?Math.round((x.move-.5)*2):0),-2,2)})}return out}
function stageMap(pack){var o={},i;for(i=0;i<pack.length;i++)o[pack[i].no]=pack[i];return o}
function simulateOne(r,rows,sc,suit,seed){var rng=prng(seed),code=chooseScenario(sc,rng),pressure=pressureInfo(rows),startScore={},cornerScore={},finishScore={},i,x,no,q,noise,earlyCost,scFit,paceHeat=0;for(i=0;i<rows.length;i++){x=rows[i];no=n(x.horse.horseNumber);q=pressure[no]||{};noise=jitter(rng);startScore[no]=x.goProb*.70+x.needLead*.12+x.jockeyFront*.07+x.flexibility*.04+(q.freeOuter||0)-q.conflict*.055+noise*.085;if(x.goProb>.55)paceHeat+=x.goProb*(.55+.45*x.needLead)}paceHeat=clamp((paceHeat-1.25)/Math.max(1,rows.length*.22),0,1.4);for(i=0;i<rows.length;i++){x=rows[i];no=n(x.horse.horseNumber);q=pressure[no]||{};noise=jitter(rng);earlyCost=(q.conflict||0)*x.goProb*(.35+.45*x.needLead)+paceHeat*x.goProb*.11;scFit=scenarioSuit(x,code,pressure);cornerScore[no]=startScore[no]*.30+x.ability*.23+x.hold*.16+x.move*.10+(1-x.fade)*.08+scFit*.15-earlyCost*.11+noise*.105;if(code==="A")cornerScore[no]+=x.goProb*.08+x.hold*.05;if(code==="C")cornerScore[no]+=x.move*.09+x.mid*.05+x.close*.06-x.goProb*.025}for(i=0;i<rows.length;i++){x=rows[i];no=n(x.horse.horseNumber);q=pressure[no]||{};noise=jitter(rng);scFit=scenarioSuit(x,code,pressure);var tripVar=(1-x.coverage)*.055*jitter(rng),stamina=(1-x.fade)*.12+x.hold*.08,late=x.move*.12+x.close*.07+x.mid*.035;finishScore[no]=scFit*.42+x.ability*.24+stamina+late+cornerScore[no]*.11-q.conflict*.025+noise*.13+tripVar;if(code==="A")finishScore[no]+=x.goProb*.055+x.hold*.035;if(code==="C")finishScore[no]+=x.move*.075+x.close*.055-x.goProb*.02}var start=stagePack(rows,startScore,0),corner=stagePack(rows,cornerScore,1),finish=stagePack(rows,finishScore,2);return{scenario:code,start:start,corner:corner,finish:finish,startMap:stageMap(start),cornerMap:stageMap(corner),finishMap:stageMap(finish),podium:finish.slice(0,3).map(function(z){return z.no})}}
...
function miniPacePreview(r){return '<div class="home-ai-preview-photo"><img src="/pace-preview.webp?v=26" alt="AI展開予想プレビュー"><span class="home-ai-preview-overlay"></span></div>'}
...
'<section class="home-card home-ai-card"><div class="home-ai-top"><span class="home-ai-bars"><i></i><i></i><i></i></span><div class="home-ai-copy"><div class="home-ai-title">AI展開予想</div><div class="home-ai-sub">AIがペースを解析し、展開・隊列を予測します</div></div>'+aiButtons+'</div>'+miniPacePreview(nx)+'</section>'+
'</main></div>'}
...
function markSymbol(p,no){for(var i=0;i<p.marks.length;i++)if(n(p.marks[i][1].horseNumber)===n(no))return p.marks[i][0];return""}
...
function renderResultGap(r,p){if(!isFinal(r))return"";var f=(r.result.finishers||[]).slice().sort(function(a,b){return n(a.finish)-n(b.finish)}),actualTop=f.slice(0,3).map(function(x){return n(x.horseNumber)}),predTop=p.plan.straight.slice(0,3).map(function(x){return n(x.horse.horseNumber)}),hit=0,i;for(i=0;i<predTop.length;i++)if(actualTop.indexOf(predTop[i])>=0)hit++;var hon=p.marks.length?p.marks[0][1]:null,honFin=hon?resultFinish(r,hon.horseNumber):0;var predLead=p.plan.start.length?p.plan.start[0].horse:null,count=actualCornerCount(r),firstLeaders=count?actualCornerLeaders(r,0):[],lastLeaders=count?actualCornerLeaders(r,count-1):[];function nums(a){return a.length?a.map(function(x){return x.horseNumber}).join("・"):"—"}return'<section class="card gap-card"><div class="section-title">レース結果とのズレ</div><div class="gap-grid"><div><small>◎の結果</small><b>'+(hon?esc(hon.horseNumber)+' '+esc(hon.name)+' → '+(honFin?honFin+'着':'着外/不明'):'—')+'</b></div><div><small>上位3頭一致</small><b>'+hit+'/3</b></div><div><small>予想ハナ → 実際序盤</small><b>'+(predLead?esc(predLead.horseNumber):'—')+' → '+nums(firstLeaders)+'</b></div><div><small>予想4角先頭 → 実際4角</small><b>'+(p.plan.corner.length?esc(p.plan.corner[0].horse.horseNumber):'—')+' → '+nums(lastLeaders)+'</b></div></div><div class="gap-order"><span>予想上位</span> '+predTop.join(" → ")+'<br><span>実着順</span> '+actualTop.join(" → ")+'</div></section>'}
...
function simPoint(run,no,t){var a=run.startMap[no]||{gap:.2,lane:0},b=run.cornerMap[no]||a,c=run.finishMap[no]||b;if(t<=.78){var u=t/.78,u2=u*u*(3-2*u);return{gap:a.gap+(b.gap-a.gap)*u2,lane:a.lane+(b.lane-a.lane)*u2}}var v=(t-.78)/.22,v2=v*v*(3-2*v);return{gap:b.gap+(c.gap-b.gap)*v2,lane:b.lane+(c.lane-b.lane)*v2}}
function simCurrentOrder(run,t){var a=run.finish.slice(),i;for(i=0;i<a.length;i++){var p=simPoint(run,a[i].no,t);a[i]={no:a[i].no,gap:p.gap}}a.sort(function(x,y){return x.gap-y.gap});return a}
function placeRunner(chip,path,board,frac,lanePx){var total=path.getTotalLength(),f=normFrac(frac),len=f*total,pt=path.getPointAtLength(len),p0=path.getPointAtLength(Math.max(0,len-2)),p1=path.getPointAtLength(Math.min(total,len+2)),rect=board.getBoundingClientRect(),sx=rect.width/200,sy=rect.height/180,dx=(p1.x-p0.x)*sx,dy=(p1.y-p0.y)*sy,mag=Math.sqrt(dx*dx+dy*dy)||1,nx=-dy/mag,ny=dx/mag,x=pt.x*sx+nx*lanePx,y=pt.y*sy+ny*lanePx;chip.style.left=x+'px';chip.style.top=y+'px'}
function drawSimulationRun(run,t){var r=state.race,board=document.getElementById('pace-board'),path=document.getElementById('course-path');if(!r||!board||!path)return;var cp=courseProfile(r),laps=Math.max(.1,n(r.distance,1200)/cp.lap),start=courseStageFrac(r,0),travel=cp.shape==="straight"?.90:laps,dir=cp.shape==="straight"?1:cp.dir,rows=state.pred?state.pred.rows:[],i,row,no,pos,frac,chip;for(i=0;i<rows.length;i++){row=rows[i];no=n(row.horse.horseNumber);pos=simPoint(run,no,t);frac=cp.shape==="straight"?clamp(start+travel*t-pos.gap,0.02,.98):normFrac(start+dir*travel*t-dir*pos.gap);chip=board.querySelector('[data-horse="'+no+'"]');if(chip)placeRunner(chip,path,board,frac,pos.lane*7)}var cur=simCurrentOrder(run,t).slice(0,3),live=document.getElementById('sim-live-podium');if(live)live.innerHTML='<span>1</span> '+cur.map(function(z,i){var h=horseByNo(r,z.no);return(i?'<i> / </i>':'')+'<b>'+(i+1)+'着 '+esc(z.no)+' '+esc(h?h.name:'')+'</b>'}).join('');var ob=document.getElementById('course-order');if(ob){var stage=t<.18?'スタート':(t<.78?'道中':'直線');ob.innerHTML='<b>'+stage+'</b>　'+cur.map(function(z){var h=horseByNo(r,z.no);return esc(z.no)+' '+esc(h?h.name:'')}).join(' → ')}}
function resetSimCounts(){var o={},hs=state.race&&state.race.horses||[],i;for(i=0;i<hs.length;i++)o[n(hs[i].horseNumber)]={w:0,s:0,t:0};state.simCounts=o;state.simDone=0}
function updateSimHud(runIndex,run){var c=document.getElementById('sim-count'),b=document.getElementById('sim-progress-bar'),e=document.getElementById('sim-elapsed'),st=document.getElementById('sim-status');if(c)c.textContent=state.simDone+' / 50';if(b)b.style.width=(state.simDone/50*100)+'%';if(e)e.textContent=state.simRunning?'×'+state.simSpeed+' 再生中':(state.simDone>=50?'完了':'待機');if(st)st.textContent=state.simDone>=50?'50回 完了':'シミュレーション '+Math.min(50,runIndex+1)+'/50';if(run&&state.simDone>0){var live=document.getElementById('sim-live-podium'),p=run.podium;if(live)live.innerHTML=p.map(function(no,i){var h=horseByNo(state.race,no);return'<b>'+(i+1)+'着 '+esc(no)+' '+esc(h?h.name:'')+'</b>'+(i<2?'<i> / </i>':'')}).join('')}}
function finishOneRun(run){var p=run.podium||[],i,z;for(i=0;i<p.length;i++){z=state.simCounts[p[i]];if(!z)continue;if(i===0)z.w++;if(i===1)z.s++;if(i===2)z.t++}state.simDone++;updateSimHud(state.simIndex,run)}
function animateOneRun(idx){if(!state.simRunning||!state.pred||!state.pred.simulation)return;var run=state.pred.simulation.runs[idx];if(!run){state.simRunning=false;updateSimHud(49,null);return}var baseMs=2400/Math.max(1,state.simSpeed),started=performance.now();function step(now){if(!state.simRunning)return;var t=clamp((now-started)/baseMs,0,1);drawSimulationRun(run,t);if(t<1){state.anim=requestAnimationFrame(step)}else{finishOneRun(run);state.simIndex=idx+1;if(state.simIndex<50){state.timer=setTimeout(function(){animateOneRun(state.simIndex)},Math.max(18,90/state.simSpeed))}else{state.simRunning=false;updateSimHud(49,run)}}}state.anim=requestAnimationFrame(step)}
function startSimulation(){if(!state.race||!state.pred)return;stopTimer();resetSimCounts();state.simIndex=0;state.simRunning=true;var btn=document.querySelector('[data-action="sim-run"]');if(btn)btn.textContent='↻ 50回やり直す';animateOneRun(0)}
function initSimulationBoard(){if(!state.pred||!state.pred.simulation||!state.pred.simulation.runs.length)return;resetSimCounts();var board=document.getElementById('pace-board'),path=document.getElementById('course-path');if(board&&path){var sf=courseStageFrac(state.race,0),total=path.getTotalLength(),sp=path.getPointAtLength(sf*total),sd=document.getElementById('course-start-dot'),sl=document.getElementById('course-start-label');if(sd){sd.setAttribute('cx',sp.x);sd.setAttribute('cy',sp.y)}if(sl){sl.setAttribute('x',Math.min(176,sp.x+4));sl.setAttribute('y',Math.max(10,sp.y-5))}}drawSimulationRun(state.pred.simulation.runs[0],0);updateSimHud(0,null)}
...
class NarStore:
    ...
    def recent_races(self, horse_name: str, cutoff: str, limit: int = 5) -> list[dict]:
        rows = self.conn.execute(
            """
            SELECT e.*, r.distance, r.weather, r.condition, r.field_size, r.prize1
            FROM entries e
            JOIN races r ON r.track=e.track AND r.date=e.date AND r.race_no=e.race_no
            WHERE e.name=? AND e.date<? AND e.finish>0
            ORDER BY e.date DESC, e.race_no DESC
                ...
                horses.append(
                    {
                        ...
                        "recentRaces": self.recent_races(e["name"], iso_date, 8),
                    }
                )
            finishers = []
            for e in entries:
...
def health():
    ...
    return {
        "status":"ok", "mode":"production-v26-sim50", "historyStarted":_history_started,
